...
状态完整保存下来，之后再原样恢复运行。只有当运行中的后端支持暂停和完整
快照，并且配置的适配器能够恢复相同的后端版本时，休眠才可用；这些检查都在
改动生命周期记录之前完成。一次成功的休眠依次完成四件事：
...
`Running`。如果失败发生在原后端停止之前，Blaze 会重新恢复原后端并复查已启用
的 guest 通信，相当于这次恢复没有发生；如果恢复失败但残留可以完整清理，
sandbox 会回到 `Hibernated`；如果无法确认清理是否彻底，替代后端的归属和操作
记录会通过 `RecoveryRequired` 保留下来，交由显式销毁收尾。
