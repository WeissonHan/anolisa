...
标准守护进程使用文件数据面。下游开发者可以在扩展软件包中实现公开源码级提供者合同，
再通过 `BlazeDaemonBuilder` 组合满足自身需求的守护进程；标准守护进程配置不能在
运行时切换实现，构建时提供者失败后也不会回退文件数据面。生命周期合同、示例、兼容
边界和暂缓能力参见
[构建时数据面提供者](docs/design/build-time-data-plane-providers_zh.md)。
...
操作记录会保存创建、销毁、检查点、回退、休眠和恢复运行的持久化边界。重启后
不会续做或接管带有未完成操作的运行实例。标准文件数据面会清理未终止的运行资源；
构建时提供者只有实现 `DataPlaneInventory`，且持久化沙箱记录、准确的
`Committed` 或 `Finalized` 租约及存活后端身份三方完全一致时，才可接管状态
干净的 `Running` 沙箱。不一致的资源会被隔离，沙箱保留为
`RecoveryRequired`。重置接口仍不可用，也不会恢复检查点。

### 检查点捕获、历史与恢复

`POST /v1/sandboxes/{id}/checkpoint` 要求所选后端支持完整快照。标准文件沙箱还
要求存储支持检查点捕获，并把可写根文件系统保存到 Blaze 的检查点目录。带有已
最终确认提供者租约的沙箱则要求构建时提供者实现可选的 `DataPlaneCheckpoint`
接口；当前提供者路径会同时冻结可写根文件系统和客户机内存，Blaze 保留后端快照
制品和一条与实现无关的持久化引用。不支持的组合会在静止后端或改变沙箱状态前
返回 HTTP 501。新捕获发布检查点格式 3；格式 1 和 2 在原后端与存储要求仍满足时
继续可读。
...
清理标记，再递归删除检查点格式 2 和 3 共用的分层载荷目录。只有本次请求创建的
清理标记全部删除，并且
...
的那一份状态重新运行。这个接口要求后端具有兼容的恢复适配器。文件检查点还要求
存储支持恢复；由提供者管理的检查点则要求当前构建时提供者实现
`DataPlaneCheckpoint`，并与记录的提供者身份一致。条件不满足时，Blaze 不改动
sandbox 的任何状态，直接返回 HTTP 501。
...
对于由提供者管理的检查点，Blaze 会在旧后端仍然运行时，根据不可变引用准备一份
独立的新租约，绝不会复用活动租约。越过停止旧后端的边界后，Blaze 使用自己持有的
后端载荷和提供者资源启动替代后端，核验就绪状态，提交新租约并发布 `Running`，
随后最终确认提供者所有权并释放旧租约。停止边界后的失败会保留清理所需的所有权
记录，并进入 `RecoveryRequired`。
...
并且配置的适配器能够恢复相同的后端版本时，休眠才可用。由提供者管理的沙箱还
要求实现 `DataPlaneSuspend`、具有已最终确认的租约，并支持客户机协议版本 1 的
`prepare_hibernate`、`reseed_rng` 和 `post_restore` 操作。这些兼容性检查都在
改动生命周期记录之前完成，因此不满足条件的组合会让 sandbox 保持运行。至于
如何把工作负载带到一致的停止点，则交给后端的“捕获前静止（quiesce）”钩子：
其默认行为是暂停后端，而自冻结后端（捕获原语自身会停止工作负载的后端）会
覆盖该钩子，因而无需单独支持暂停。

在标准文件路径上，一次成功的休眠依次完成四件事：
...
在提供者路径上，提供者会在后端快照采用的同一静止边界冻结不可变的可写根文件
系统和客户机内存内容。后端停止后，Blaze 会停止并释放活动租约；已休眠沙箱只
保留不可变的休眠引用。

恢复的顺序同样是先校验、再切换。Blaze 会先核对清单记录的身份、文件集合是否
完全一致以及每个文件的摘要，确认无误后才启动替代后端。manager 先取得这个新
后端的归属。标准文件路径可以等待可选的客户机通信就绪；提供者路径每次都会准备
全新的独占租约，并且必须在发布 `Running` 前完成随机熵注入和客户机时钟校正。
如果在替代后端启动之前失败，sandbox 会回到 `Hibernated`，可以重试；
如果无法确认替代后端的清理是否彻底，其归属和操作记录会通过 `RecoveryRequired`
保留下来，交由显式销毁收尾。

标准文件路径会在休眠期间保留存储空间。提供者路径不会保留活动租约，并在每次
恢复运行时准备新租约。两条路径都会保留最近一次可恢复的镜像或引用，直到下一次
休眠替换它，或者显式销毁将其删除。daemon 重启后不会自动续做中断的休眠或恢复
操作。
