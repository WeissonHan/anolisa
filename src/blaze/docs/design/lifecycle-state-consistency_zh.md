...
操作、后端持有者仍存活且身份一致，并且后端支持完整捕获。标准文件路径还要求存储
支持捕获；提供者路径要求已最终确认的租约和 `DataPlaneCheckpoint`，且当前实现必须
同时持有可写根文件系统和客户机内存。组合不受支持时，在静止后端或改变生命周期状态
前返回 `501 Not Implemented`。
...
镜像、后端、后端版本和快照类型一致，并要求后端明确支持恢复。文件检查点还要求
存储支持恢复；提供者管理的检查点要求匹配的提供者实例实现
`DataPlaneCheckpoint`。不受支持的组合会返回 `501 Not Implemented`，当前后端和
生命周期记录保持不变。
