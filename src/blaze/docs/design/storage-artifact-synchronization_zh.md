...
收到终止信号时，守护进程会停止两个监听器，并拒绝新的模板导入。随后，
守护进程会并行排空已接收的 HTTP/1 连接并停止周期存储同步。无法取消的
提供方工作会继续持有对应沙箱的锁，直至完成；两条关闭路径的失败都会保留。

两条路径结束后，守护进程再等待已经登记的模板导入。把取消信号继续传入
管理器操作，以及释放守护进程持有的全部运行时资源，仍属于独立的关闭职责。
