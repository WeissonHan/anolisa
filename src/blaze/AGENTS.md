...
- **Backend traits**: All backend-specific process management is behind `BackendSpawner` (`spawn`, `probe`, `cleanup_orphan`, and defaulted `restore` and `release_rootfs`) and `BackendInstance` (`backend`, `try_wait`, `kill`, and defaulted `pause`, `resume`, `snapshot`). Adding a new backend means implementing the required methods and registering it in `daemon::build_spawners()`; the defaulted ones report `UnsupportedOperation` until overridden, except `release_rootfs`, which defaults to a no-op. `release_rootfs` is a daemon-driven step invoked wherever storage is released, because a hibernated sandbox reaches neither `kill` nor `cleanup_orphan`.
