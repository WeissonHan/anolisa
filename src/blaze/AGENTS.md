...
- **Lifecycle state machine**: The persisted model contains 9 states. Current
  managed routes implement create, destroy, and RecoveryRequired cleanup.
  Checkpoint and reset routes are reserved and return `501` without advancing
  state. Transitions are enforced by `blaze_core::lifecycle`; do not bypass
  them through direct field mutation.
