...
fn list_sandboxes(state: &Arc<ServerState>) -> Result<Response<Full<Bytes>>> {
    let visible = state
        ...
        .list()?
        .into_iter()
        .filter(|sandbox| sandbox.state != SandboxState::Destroyed)
        .collect::<Vec<_>>();
    json_ok(&visible)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn public_list_excludes_destroyed_tombstones_without_changing_id_semantics() {
        let temp = tempfile::tempdir().expect("temp");
        let state = state(temp.path());
        let id = Uuid::parse_str("00000000-0000-4000-8000-000000000001").expect("id");
        let body = serde_json::to_vec(&json!({
            "id": id,
            "workload_class": "agent-tool",
            "image_digest": "sha256:list-contract"
        }))
        .expect("body");
        ...
        let listed = response_json(
            dispatch(&Method::GET, "/v1/sandboxes", "", Vec::new(), &state)
                .await
                .expect("list running"),
        )
        .await;
        ...
        assert_eq!(
            dispatch(&Method::DELETE, &path, "", Vec::new(), &state)
                .await
                .expect("destroy")
                .status(),
        ...
        let tombstone = response_json(
            dispatch(&Method::GET, &path, "", Vec::new(), &state)
                .await
                .expect("get tombstone"),
        )
        .await;
        assert_eq!(tombstone["state"], "destroyed");
        assert_eq!(
            dispatch(&Method::DELETE, &path, "", Vec::new(), &state)
                .await
        ...
        assert_eq!(
            route_status(&state, Method::POST, "/v1/sandboxes", &body).await,
            StatusCode::CONFLICT
        );
        assert_eq!(state.manager.list().expect("internal tombstones").len(), 1);
    }
