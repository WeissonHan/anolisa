...
pub enum BlazeDaemonError {
    ...
    #[error("conflict: {0}")]
    #[allow(dead_code)] // Used by lifecycle handlers introduced in the next stacked PR.
    Conflict(String),

    #[error("request body too large: {actual} bytes exceeds {limit}")]
    #[allow(dead_code)] // Used by the bounded API body reader in the final stacked PR.
    PayloadTooLarge { actual: usize, limit: usize },

    #[error("operation requires recovery: {0}")]
    #[allow(dead_code)] // Used by owned runtime cleanup introduced later in this stack.
