...
impl GuestClient {
    ...
    pub async fn exec(
        ...
        if command.is_empty() {
            return Err(GuestError::Protocol("exec command is empty".to_string()));
        }
...
fn operation_timeout(guest_timeout_secs: u32) -> Duration {
    Duration::from_secs(u64::from(guest_timeout_secs).saturating_add(10))
}

async fn read_line(stream: &mut UnixStream, limit: usize) -> Result<Vec<u8>> {
...
mod tests {
    ...
    async fn read_and_write_response_timeouts_are_bounded() {
        ...
        assert!(matches!(write_error, GuestError::Timeout(_)));
        server.await.expect("server task");
    }
