...
async fn copy_for_restore(
    ...
    let mut source_file = source_options.open(source).await.map_err(|error| {
    ...
    loop {
        let count = source_file
            .read(&mut buffer)
            .await
            .map_err(|error| storage_error(format!("read restore source: {error}")))?;
        if count == 0 {
            break;
        }
        destination_file
            ...
            .await
            .map_err(|error| storage_error(format!("write restore stage: {error}")))?;
        if first_chunk {
            first_chunk = false;
            crate::failpoint::pause("storage-restore-copy-after-chunk").await;
        }
    }
    destination_file
        .sync_all()
        .await
        .map_err(|error| storage_error(format!("sync restore stage: {error}")))
}
...
mod tests {
    ...
    #[cfg(feature = "test-failpoints")]
    #[tokio::test]
    async fn cancelled_partial_copy_is_removed() {
        ...
        cancel_at(provider, slot, source, "storage-restore-copy-after-chunk").await;
