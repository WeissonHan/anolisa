...
impl SandboxManager {
    ...
    pub async fn checkpoint(&self, id: Uuid) -> Result<CheckpointMetadata> {
        ...
        crate::failpoint::pause("checkpoint-after-begin");
        ...
        if let Err(error) = paused {
            self.recover_paused_checkpoint(
                id,
                &backend,
                runtime.guest.as_ref(),
                Some(&checkpoint_id),
            )
            .await?;
        ...
        crate::failpoint::pause("checkpoint-after-publish-before-head");
        let store = self.checkpoints.clone();
        ...
        if let Err(error) = head_updated {
            self.recover_paused_checkpoint(id, &backend, runtime.guest.as_ref(), None)
                .await?;
            return Err(error);
        }
        crate::failpoint::pause("checkpoint-after-head");
    ...
    pub async fn rollback(self: &Arc<Self>, id: Uuid, checkpoint_id: &str) -> Result<()> {
        ...
        crate::failpoint::pause("rollback-after-begin");
        ...
        crate::failpoint::pause("rollback-after-rootfs");
        ...
        crate::failpoint::pause("rollback-after-head");
