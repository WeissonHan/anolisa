...
fn copy_template_transactional(
    ...
    let outcome = (|| {
        ...
        if value
            ...
            .is_none()
        {
            value["memfile_size"] = json!(4_u64 * 1024 * 1024 * 1024);
        }
        std::fs::write(&template_json, serde_json::to_vec_pretty(&value)?)?;
        for required in ["snapshot_file", "meta.json"] {
        ...
        ensure_template_alias(
            &staging,
            "memory.img",
            &["memfile.img", "memory-rw.img", "memory_rw.img"],
        )?;
        let rootfs_layers = value
            .get("rootfs_layers")
            .and_then(serde_json::Value::as_array)
            .cloned()
            .unwrap_or_default();
        if rootfs_layers.is_empty() {
            ensure_template_alias(&staging, "rootfs.img", &["rootfs-rw.img", "rootfs_rw.img"])?;
        } else {
            for layer in rootfs_layers {
                let layer = layer.as_str().ok_or_else(|| {
                    BlazeDaemonError::BadRequest(
                        "template rootfs_layers entries must be strings".to_string(),
                    )
                })?;
                validate_name(layer, "rootfs layer")?;
                if !staging.join(layer).is_file() {
                    return Err(BlazeDaemonError::BadRequest(format!(
                        "template source is missing rootfs layer {layer}"
                    )));
                }
            }
        }
...
fn ensure_template_alias(directory: &Path, canonical: &str, aliases: &[&str]) -> Result<()> {
    let destination = directory.join(canonical);
    if destination.is_file() {
        return Ok(());
    }
    if let Some(source) = aliases
        .iter()
        .map(|alias| directory.join(alias))
        .find(|path| path.is_file())
    {
        std::fs::copy(source, destination)?;
        return Ok(());
    }
    Err(BlazeDaemonError::BadRequest(format!(
        "template source is missing {canonical}"
    )))
}
...
mod tests {
    ...
    fn import_publishes_canonical_artifacts_and_defaults() {
        ...
        std::fs::write(source.join("snapshot_file"), b"snapshot").expect("snapshot");
        std::fs::write(source.join("meta.json"), b"{}").expect("metadata");
        std::fs::write(source.join("memfile.img"), b"memory").expect("memory alias");
        std::fs::write(source.join("rootfs-rw.img"), b"rootfs").expect("rootfs alias");
        ...
        assert_eq!(metadata["memfile_size"], 4_u64 * 1024 * 1024 * 1024);
        assert_eq!(
            std::fs::read(destination.join("memory.img")).expect("canonical memory"),
            b"memory"
        );
        assert_eq!(
            std::fs::read(destination.join("rootfs.img")).expect("canonical rootfs"),
            b"rootfs"
        );
        assert_eq!(
