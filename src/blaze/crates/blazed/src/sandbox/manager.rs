...
//! Core sandbox create, destroy, and runtime ownership.
...
/// Inputs already parsed and policy-evaluated by the caller.
#[derive(Debug, Clone)]
...
/// Result of a managed sandbox create request.
#[derive(Debug, Clone)]
...
impl SandboxManager {
    ...
    async fn start_runtime(
        ...
        if guest_enabled(selected_backend, backend) {
    ...
    fn retain_recovery_runtime(
        ...
        self.runtimes
            ...
            .map_err(|_| poisoned("runtimes"))?
            .entry(id)
            .or_insert_with(|| Arc::new(AsyncMutex::new(SandboxRuntime { storage, backend })));
        Ok(())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn create_and_idempotent_destroy() {
