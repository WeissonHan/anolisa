...
- gVisor 后端通过 containerd 的镜像与快照服务，把每个 sandbox 根植于一个普通 OCI 镜像并
  给它自己的可写层。**默认启用**，包内 `[containerd]` 段指向
  `/run/containerd/containerd.sock`。sandbox 进程仍归 blaze 持有，完整生命周期不受影响。
  把 `address` 置空即关闭，此时需要自行准备 `<images_dir>/gvisor-rootfs` 这份共享只读
  目录——没有任何包提供它。
