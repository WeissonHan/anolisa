...
- Sandbox 创建、列出、查看和销毁 API，并通过持久化生命周期操作与显式恢复状态处理失败。
- 有界 Guest 命令执行和文件读写 API，并具有明确的载荷与请求限制。
- 可配置的运行时 warm pool，支持异步补充、容量报告、清理以及可选的就绪虚拟机。
- 可选的 Firecracker sandbox 独立网络，并对已分配资源承担清理责任。
- 带完整性校验和谱系关系的 checkpoint 制品，支持 rollback 与 prune。
- 使用持久化制品且可在守护进程重启后恢复的 sandbox hibernate 与 resume。
- 具有有界导入校验和 daemon 查询 API 的事务式运行制品目录。
- 可选的运行中 sandbox 存储定期同步。

### 变更

- File provider 现在创建自包含运行时快照，以额外容量换取每次 restore 的独立性。
- 运行时进程和 host 资源由 backend 所有权接口持有，中断的清理操作可以重试。
- 启用网络的 VM snapshot 会恢复 capture 时的网络 slot，保持 guest 设备名稳定。
