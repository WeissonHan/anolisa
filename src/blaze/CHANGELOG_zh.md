...
- 有界 Guest 命令执行和文件读写 API，可配置载荷与请求限制。
- 运行时 warm pool，支持异步补充、容量报告、清理以及可选的就绪虚拟机。
- 带完整性校验和谱系关系的 checkpoint，支持 rollback、prune、hibernate 与 resume。
- 事务式模板导入，以及 provider 持有的运行时制品定期同步。
