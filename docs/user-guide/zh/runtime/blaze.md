...
捕获始终要求所选后端支持完整快照。标准文件路径还要求存储支持检查点捕获，并由
配置的存储提供程序保存可写根文件系统。带有已最终确认提供者租约的沙箱则要求实现
`DataPlaneCheckpoint`；当前提供者路径必须同时持有可写根文件系统和客户机内存。
Firecracker 和内置 `mock` 后端支持完整捕获，Bubblewrap 和其他进程后端尚不支持。
不支持的组合会在静止后端或修改生命周期记录前返回 HTTP 501。
...
对于已最终确认的提供者租约，Blaze 仍把后端制品保存在 `backend/` 下，而
`DataPlaneCheckpoint` 会在同一静止边界冻结不可变的可写根文件系统和客户机内存。
持久化清单保存与实现无关的引用和摘要，不要求存在 `storage/rootfs.snap`；管理
接口响应不会返回这条所有权记录。

成功响应包含已发布清单的管理接口视图。现有 `checkpoint_id` 和 `instance_id`
字段与
...
整棵镜像目录。格式 1 和 2 在原后端与存储要求仍满足时继续可读和可恢复；新捕获
一律发布格式 3。格式 3 可以在持久化状态中保存与实现无关的提供者所有权记录，
但管理接口响应不会返回这条记录及其中的不透明标识。

可以通过 `GET /v1/sandboxes/{id}/checkpoints` 查询已提交的历史。每个列表项
包含 `id`、`parent`、`created_at`、Blaze 检查点目录内制品的逻辑大小
`size_bytes`、`is_head`、`on_head_chain` 和 `provider_managed`。该列表只提供
摘要，不会重复捕获响应中的完整制品清单。
...
sandbox 完全一致，并具有兼容的后端恢复适配器。文件检查点还要求存储支持恢复；
提供者管理的检查点则要求匹配的提供者实例实现 `DataPlaneCheckpoint`。不支持的
组合会在停止当前运行环境前返回 HTTP 501。
...
对于提供者管理的检查点，Blaze 会在旧后端仍运行时根据不可变引用准备全新的独占
租约，并在停止边界前记录替代租约。随后使用 Blaze 持有的后端制品和已准备的提供者
资源启动替代后端，只在身份与就绪检查通过后发布替代结果。最后提交并最终确认新租约，
再释放旧租约；整个过程不会复用活动租约。
...
沙箱的身份和可恢复状态都保留下来；与检查点捕获的区别是捕获之后后端继续运行，而休眠
之后后端会被停止。
...
由提供者管理的沙箱还要求实现 `DataPlaneSuspend`、具有已最终确认的活动租约，
并支持客户机协议版本 1 的 `prepare_hibernate`、`reseed_rng` 和 `post_restore`。

在标准文件路径上，一次成功的休眠依次完成：记录操作意图、为捕获而静止后端、把
后端载荷和客户机
...
无误后才启动替代后端。Blaze 会先取得该后端的归属。标准文件路径可以等待可选的
客户机通信就绪；提供者路径会准备全新的独占租约，并在发布 `Running` 前完成随机
熵注入和客户机时钟校正。文件损坏或缺失会在任何后端启动之前就被拒绝。
...
标准文件路径会在休眠期间保留存储空间。提供者路径不会保留活动租约，而是保留
不可变的休眠引用，并在每次恢复时准备新租约。两条路径都会保留最近一次可恢复的
镜像或引用，直到下一次休眠替换它或销毁将其删除。daemon 重启后，已经完成的休眠
仍可恢复，但中断的休眠或恢复操作不会自动续做，而是以 `RecoveryRequired` 保留，
等待显式销毁。
